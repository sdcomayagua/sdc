window.SDC_CART = (() => {
  const ST=window.SDC_STORE, U=window.SDC_UTILS;

  function updateCount(){
    let count=0; for(const it of ST.cart.values()) count+=it.qty;
  ...
  function close(){ U.$("cartModal").classList.remove("open"); }

  function render(){
    const el=U.$("cartItems"); el.innerHTML="";
    if(ST.cart.size===0){ el.innerHTML=`<div class="note">Tu carrito está vacío.</div>`; return; }
    ...
    for(const [id,it] of ST.cart.entries()){
      ...
      row.innerHTML=`
        ...
        <div style="flex:1">
          ...
          <div style="margin-top:6px;font-weight:900">${window.SDC_UTILS.money(p.precio, window.SDC_CONFIG.CURRENCY)} <span class="mut">x ${it.qty}</span></div>
        </div>
        <div class="qty">
      ...
      row.querySelectorAll("button").forEach(b=>{
        b.onclick=()=>{
          ...
          const item=ST.cart.get(pid); if(!item) return;
          const stock=Number(item.p.stock||0);

          if(act==="minus"){ item.qty=Math.max(1,item.qty-1); ST.cart.set(pid,item); }
          if(act==="plus"){ if(item.qty+1>stock){U.toast("No hay stock suficiente");return;} item.qty+=1; ST.cart.set(pid,item); }
          if(act==="del"){ ST.cart.delete(pid); }

          updateCount(); render(); window.SDC_DELIVERY.compute();
        };
      });
      el.appendChild(row);
    }
