(()=>{
/* ================= CONFIG ================= */
// Backend (Google Apps Script) – fuente de verdad
...
// Catálogo estático (se actualiza con GitHub Actions desde Google Sheets)
// Ventaja: carga más rápida, menos errores por CORS / rate-limit.
...
async function loadAPI(){
 // 1) Intenta catálogo estático del repo (actualizado por GitHub Actions)
 try{
  const r0=await fetch(`${CATALOG_URL}?v=${Date.now()}`,{cache:"no-store"});
  if(r0.ok){
   const j=await r0.json();
   if(j && (j.productos||j.envios||j.municipios_hn||j.zonas_comayagua_ciudad)) return j;
  }
 }catch(_e){}

 // 2) Fallback a Apps Script en vivo
